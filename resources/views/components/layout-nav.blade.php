<x-layout>
    <div id="navbar" class="w-full">
        <nav class="bg-pink-200 py-5">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-1 md:space-x-0 rtl:space-x-reverse">
                    <button  data-drawer-target="sidebar"  data-drawer-toggle="sidebar" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-pink-500 rounded-lg md:hidden hover:bg-pink-100" aria-controls="sidebar" aria-expanded="false">
                        <span class="sr-only">Open main menu</span>
                        <x-ei-navicon class="size-12"/>
                    </button>
                </div>
                ...
                <div class="flex items-center mr-5">
                    <button class="text-white font-extrabold">
                        <x-ei-search class="size-10"/>
                    </button>
                    <button class="text-white font-extrabold">
                        <x-ei-user class="size-10"/>
                    </button>
                    <button class="text-white font-extrabold">
                        <x-ei-cart class="size-10"/>
                    </button>
        ...
        <nav class="hidden md:block bg-pink-200">
            <div class="flex items-center justify-center">
                <div class="flex items-center justify-center w-full">
                    <ul class="flex flex-row font-medium p-0 space-x-4 w-auto">
                        @foreach($menus as $m)
                            <li>
                                <a href="{{ $m['link'] }}"  class="navbar-link">
                                    @if($m['icon'] != '')
                                    <x-dynamic-component :component="$m['icon']" class="size-6 mr-1"/>
                                    @endif
                                    <span>{{ $m['label'] }}</span>
                                </a>
                            </li>
    ...
    <aside id="sidebar" class="absolute top-50 left-0 z-40 w-64 h-screen transition-transform -translate-x-full" aria-label="Sidebar">
        <div class="h-full px-3 py-4 overflow-y-auto bg-pink-50">
            <ul class="space-y-2 font-medium list-none">
                @foreach($menus as $m)
                    <li>
                        <a href="{{ $m['link'] }}" class="nav-link group">
                            @if($m['icon'] != '')
                            <x-dynamic-component :component="$m['icon']" class="size-8" />
                            @endif
    ...
    <div class="container mx-auto mt-3">{{ $slot }}</div>
</x-layout>
